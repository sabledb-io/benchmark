use crate::{CommonError, ParserError, StringUtils};
use bytes::BytesMut;
use std::rc::Rc;
...
impl RequestParser {
    ...
    fn read_len(&self, buffer: &[u8]) -> Result<(usize, usize), CommonError> {
        let Some(pos) = StringUtils::find_subsequence(buffer, b"\r\n") else {
            return Err(CommonError::Parser(ParserError::NeedMoreData));
        };

        if pos > 20 {
            // we allow max of 20 chars of string length
            return Err(CommonError::Parser(ParserError::BufferTooBig));
        }

        let len_as_str = String::from_utf8_lossy(&buffer[0..pos]);
        let Ok(length) = len_as_str.parse::<usize>() else {
            return Err(CommonError::Parser(ParserError::ProtocolError(
    ...
    pub fn parse(&mut self, buffer: &[u8]) -> Result<ParseResult, CommonError> {
        // sanity check
        if buffer.is_empty() {
            return Err(CommonError::Parser(ParserError::NeedMoreData));
        }
            ...
            ParserState::Initial => match buffer[0] {
                b'*' => {
                    ...
                    let Some(curpos) = self.curpos.checked_add(1 + bytes_to_skip) else {
                        return Err(CommonError::Parser(ParserError::Overflow));
                    };
                    self.curpos = curpos;
    ...
    fn parse_inline_string(&mut self, buffer: &[u8]) -> Result<ParseResult, CommonError> {
        ...
        Ok(ParseResult {
            command: Rc::new(StringUtils::split(&mut command_string)?),
            // its OK to add `+2` here, since we managed to find `\r\n` above
            bytes_consumed,
        })
    }

    fn parse_array_of_bulk_strings(&mut self, buffer: &[u8]) -> Result<ParseResult, CommonError> {
        ...
        while self.expected_items > 0 {
            ...
            // locate the `$`
            if buffer[curpos] != b'$' {
                return Err(CommonError::Parser(ParserError::ProtocolError(format!(
...
mod tests {
    ...
    fn test_incomplete_inline_string() {
        ...
        assert_eq!(result.command.first().unwrap(), "helloworld");
        ...
        assert_eq!(result.command.first().unwrap(), "anotherstring");
    }
    ...
    fn test_string_with_more_than_one_inline_message() {
        ...
        assert_eq!(result.command.first().unwrap(), "SET");
        assert_eq!(result.command.get(1).unwrap(), "KEY");
        assert_eq!(result.command.get(2).unwrap(), "VALUE");
        ...
        assert_eq!(result.command.first().unwrap(), "INFO");
    }
    ...
    fn test_inline_message_with_quotes() {
        ...
        assert_eq!(result.command.first().unwrap(), "SET");
        assert_eq!(result.command.get(1).unwrap(), "KEY");
        assert_eq!(result.command.get(2).unwrap(), "VALUE W SPACE");
    }
    ...
    fn test_read_incomplete_array_strings_in_chunks() {
        ...
        assert_eq!(result.command.len(), 4);
        assert_eq!(result.command.first().unwrap(), "SET");
        assert_eq!(result.command.get(1).unwrap(), "KEY");
        assert_eq!(result.command.get(2).unwrap(), "VALUE");
        assert_eq!(result.command.get(3).unwrap(), "VALUE2");
        ...
        assert_eq!(result.command.first().unwrap(), "inelinestring");
    }
